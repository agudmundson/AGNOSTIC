...
class SingleCNN():
	...
	def __init__(self, basedir, trndir, modeldir, bsize, target, domain='Time', verbose=False, loss_fx='MeanSquaredError'):
		...
		# self.loss_fx    = losses.MeanSquaredError()
		...
		self.loss_fx    = self.weighted_MSE 												# Weighted Mean Squared Error
	...
	def tf_train_freq(self, fname, target='Metab'):
		with np.load(fname) as data:
			return data['x'], data[target] * 10, ((data['mask'] * 10) + 1)
	...
	def tf_valid_freq(self, fname, target='Metab', N=7200):
		with np.load(fname) as data:
			return data['x'][:N,:,:,:], data[target][:N,:,:,:] * 10, ((data['mask'][:N,:,:,:] * 10) + 1) 
	...
	def build_model(self):																							
		...
		l9   =                     self.conv_str2(160,    l8       )						#      8,  1, 192  ( 9)
		l10  = self.conv_str2(192, self.conv_str1(160,    l9       )) 						#      4,  1, 160  (10)
		l11  = self.tran_str2(160, self.conv_str1(160,    l10      )) 						#      8,  1, 128  (11)
	...
	@tfFunction
	def valid_step_quick(self, x, y, sw): 													# Custom ValidStep
		ypred    = self.model(x, training=False) 											# Get Predictions - No Updates
		loss     = self.loss_fx(y, ypred, sw) 											 	# Loss
		return loss
...
if __name__ == '__main__':
	...
	mname       = 'AG_{}_CNN_{}'.format(target , domain) 								# Current Model Name
	...
	x,y,sw        = NN.tf_valid_time(validname, N=1800, target=target) 						# Take 1800 Unseen Examples
	data_valid    = Dataset.from_tensor_slices((x,y,sw)) 					 				# Create TF Dataset
	...
	if os.path.exists(logname) and override == False: 										# Log File Exists																	
		...
		for step, (x_,y_,sw_) in enumerate(data_valid): 									# Iterate over Dataset
			Loss_Best[step] = NN.valid_step_quick(x_,y_,sw_) 								# No Weight Updates
	...
	for ii in range(data_idx.shape[0]): 													# Iterate over Epochs
		...
		x,y,sw         = NN.tf_train_time(trainname, target=target) 						# Load Training Data
		load_time      = t0.time() - load_time												# Time  to Load

		data_train     = Dataset.from_tensor_slices((x,y,sw)) 					 			# Convert Training Data to TF Dataset
		data_train     = data_train.batch(batch_size)  										# Create Batches

		## Training
		train_time      = t0.time() 														# Time to Train
		for step, (x_,y_,sw_) in enumerate(data_train): 									# Iterate over Train Steps
			Loss_Train_[step] = NN.train_step_quick(x_,y_,sw_) 								# Fast TrainStep (Operate Directly Tensorflow on Graph)
		...
		for step, (x_,y_,sw_) in enumerate(data_valid): 									# Iterate over Valid Steps
			Loss_Valid_[step] = NN.valid_step_quick(x_,y_,sw_)								# Fast ValidStep (Operate Directly Tensorflow on Graph)
		...
		if Loss_Valid[ii] < Loss_Best:	 													# Save Weight if Validation Loss Improves
			...
			if Loss_Best  < .035: #  Time = .003; Freq = .035
